...
def send_funds():
    ...
    if not all([address, amount, currency]):
        return jsonify({"error": "address, amount, and currency are required"}), 400

    response = requests.post(
        f"{LUNO_API_URL}/api/1/send",
        json={"address": address, "amount": amount, "currency": currency},
    ...
    response = requests.post(
        f"{LUNO_API_URL}/api/1/create_address",
